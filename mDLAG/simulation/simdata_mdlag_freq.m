function [seq, params] = simdata_mdlag_freq(N, T, binWidth, yDims, xDim, ...
    hyperparams, snr, covType, gp_params, varargin)
% 
% [seq, params] = simdata_mdlag_freq(N, T, binWidth, yDims, xDim, ...
%                               hyperparams, snr, covType, gp_params, ...)
%
% Description: Generate simulated data according to a (multi-group) Delayed
%              Latents Across Groups (mDLAG) model. Sequences are generated 
%              approximately in the frequency domain. The true covariances 
%              are thus circulant. Only the complex-conjugate portion of 
%              the frequency domain latents should be considered.
%
% Arguments:
%
%     Required:
...
seq = generate_obs_mdlag(seq, params, 'latentfield', timefield, ...
                         'obsfield', obsfield);        