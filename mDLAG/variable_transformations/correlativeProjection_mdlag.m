function seq = correlativeProjection_mdlag(seq, params, groups, varargin)
%
% seq = correlativeProjection_mdlag(seq, params, groups, ...)
%
...
%                xcorr  -- (2*xDim x T) array; trajectories projected onto 
%                          correlative modes. The first xDim latents 
...
[~, U, V, ~] = correlativeModes_mdlag(params, groups, 'zerolag', zerolag);
...
for groupIdx = 1:length(groups)
    ...
    if orth
        ...
        Xcorr = V{groupIdx}' * params.C.means{groups(groupIdx)} * X;
    else
        % Project onto uncorrelated basis
        Xcorr = U{groupIdx}' * params.C.means{groups(groupIdx)} * X;
    end
