...
%
% Description: Initialize mDLAG parameters for fitting.
%
% Arguments:
%
%     Required:
...
for groupIdx = 1:numGroups
    CC_m = sum(cat(3,params.C.moments{groupIdx}{1:yDims(groupIdx)}),3);
...
switch covType
    ...
    case 'sg'
        ...
        params.gamma = (binWidth ./ startTau).^2 .* ones(1, xDim);
        % GP center frequency
        params.nu = startNu * binWidth * ones(1, xDim);
        % GP noise variance
        params.eps = startEps .* ones(1, xDim);
    ...
    case 'exp'
        % GP timescale
        % Assume binWidth is the time step size.
        params.gamma = binWidth ./ startTau .* ones(1, xDim);
        % GP noise variance
        params.eps = startEps .* ones(1, xDim);
