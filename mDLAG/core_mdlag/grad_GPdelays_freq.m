function [f,df] = grad_GPdelays_freq(p,precomp,const)
%
% [f, df] = grad_GPdelays_freq(p, precomp, const)  
%
...
for j = 1:length(precomp.Tu)
    ...
    QQXXCPhiC = repmat(permute(conj(Q),[1 3 2]),[1 xDim 1]) ...
             .* repmat(permute(Q,[3 1 2]),[xDim 1 1]) ...
             .* precomp.Tu(j).XXCPhiC;

    QYXPhiC = repmat(permute(Q,[3 1 2]),[size(precomp.Tu(j).YXPhiC,1) 1 1]) ...
           .* precomp.Tu(j).YXPhiC;
    ...
    df = df + permute(...
                  real(sum(...
                      repmat(permute(ifreqs,[2 3 1]),1,xDim,1).*(...
                          sum(QQXXCPhiC,1) - sum(QYXPhiC,1)...
                      ),...
                  3)),...
              [2 1 3]);
    % Objective update
    f = f - real(0.5.*sum(QQXXCPhiC,'all') - sum(QYXPhiC,'all'));
