function [f,df] = grad_GPparams_exp_freq(p,precomp,const)
%
% [f, df] = grad_GPparams_exp_freq(p, precomp, const)  
%
% Description: Gradient computation for exponential function GP parameter
...
for j = 1:length(precomp.Tu)
    ...
    S = (1 - const.eps) .* (2*gamma) ./ (gamma.^2 + (2*pi.*freqs).^2) ...
        + const.eps; % (T x 1) vector of diagonal elements
    S_inv = 1./S;
    logdet_S = sum(log(S));
    
    dS_dgamma = (1 - const.eps) ...
        .* 2 .* ((2*pi.*freqs).^2 - gamma.^2) ...
        ./ (gamma.^2 + (2*pi.*freqs).^2).^2;  % (xDim x 1) vector of diagonal elements
