% set_consts_smdlag_modelselection.m
%
...
%% smDLAG fitting parameters

xDim_fit = 8;      % Set to some value higher than the ground truth
nyqperiod = 50;    % Minimum sampling period (in ms) that we can get away
                   % with when choosing the number of inducing points.
                   % Empirically, for the squared exponential function, 
                   % when tau = 50 ms, nearly 100% of the power spectrum 
                   % is contained within 10 Hz (i.e., ~20 Hz Nyquist rate).
...
freqParam = 100;      % Store intermediate estimates every freqParam iterations
