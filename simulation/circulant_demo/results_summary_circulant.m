% results_summary_circulant.m
%
% Description: This script displays mDLAG-frequency domain results on 
%              circulant data, which should exhibit model-data match.
%
% Author: 
...
load(dataFile);
resultFile = sprintf('%s/mdlag_results_circulant_freq.mat', resultDir);
load(resultFile);
...
% Estimate via frequency domain inference

% Take the FFT of each neuron's activity on each trial
seqTrue = fftseq(seqTrue,'y','yfft');
% Inference step
[seqEst,~,~] = inferX_freq(seqTrue, estParams);
% Convert estimated latents back to the time domain
seqEst = freq2time_mdlag(seqEst, estParams, 'infield', 'xfft', 'outfield', 'xsm');
...
% Leave-group-out predictive performance (via the frequency domain)
[R2, MSE] = pred_mdlag_freq(seqTrue, estParams);
