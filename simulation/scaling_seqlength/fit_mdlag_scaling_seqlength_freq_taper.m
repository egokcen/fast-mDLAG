% fit_mdlag_scaling_seqlength_freq_taper.m
%
% Description: Fit mDLAG models via frequency domain approximation to 
%              data with increasing sequence (trial) lengths. Apply
%              a taper to the training data to see if it mitigates bias in
%              GP parameter estimates.
%
% Author: 
...
parfor runIdx = 1:numRuns
    ...
    for i = 1:numPartitions
        ...
        % Apply the taper
        seqSub = apply_taper(seqSub, @(T) hamming(T, 'periodic'), 'y');
...
for runIdx = 1:numRuns
    ...
    for i = 1:numPartitions
        ...
        currSaveFile = sprintf('%s/T%04d_freq_hamming.mat', currSaveDir, T);
