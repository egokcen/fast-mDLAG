...
%
% Description: Plot intermediate values stored throughout smDLAG model
...
%         Zs       -- (1 x numIters) cell array; estimated inducing 
%                     point locations after each EM iteration.
...
if xDim > 0
    ...
    % Inducing point location progress
    figure;
    for i = 1:xDim
        subplot(1,xDim,i);
        hold on;
        Zs = [];
        for k = 1:length(trackedParams.Zs)
            Zs = [Zs; trackedParams.Zs{k}{i}];
        end
        plot(freqParam.*(0:length(trackedParams.Zs)-1), ...
             binWidth.*(Zs-1), ...
             'linewidth', 1.5);
        xlabel('# Iterations');
        ylabel(sprintf('Inducing point locations%s',units), 'fontsize', fontsize);
        str = sprintf('$${\\mathbf w}_{%d,:}$$',i);
