function [R2, MSE] = pred_smdlag(seq, params)
%
% [R2, MSE] = pred_smdlag(seq, params)
%
% Description: Performs leave-group-out prediction using an existing smDLAG
%              model.
%
% Arguments:
%
...
for groupIdx = 1:numGroups
    ...
    % Infer inducing points W given source groups
    paramsSource = getSubsetGroups_params(params,sourceGroups);
    seqSource = getSubsetGroups_seq(seq,yDims,sourceGroups);
    [seqSource,~,~] = inferXW(seqSource,paramsSource,'inferX',false);
    
    % Infer latent variables X for target group, given inducing points
    paramsTarget = getSubsetGroups_params(params,targetGroup);
    seqTarget = predX_smdlag(targetGroup,paramsTarget,seqSource);
