function params = init_smdlag(seq,yDims,xDim,binWidth,varargin)
%
% params = init_smdlag(seq,yDims,xDim,binWidth,...)
%
% Description: Initialize smDLAG parameters for the variational EM 
%              algorithm.
%
% Arguments:
%
...
%                   (default: [])
%     S          -- if int, then initialize all inducing points with the 
%                   same number of locations.
%                   if (1 x xDim) int array, then initialize each latent j
%                   with S(j) (default: max([seq.T]))
...
params.Z = cell(xDim,1);
if length(S) > 1
    for j = 1:xDim
        params.Z{j} = linspace(1,max([seq.T]),S(j));
    end
else
    params.Z = repmat({linspace(1,max([seq.T]),S)},xDim,1);
end